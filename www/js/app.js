/**
 * ============================================================================
 * BLOOMFLOW / LITTLE BLOOM PRO - CORE JAVASCRIPT APPLICATION ENGINE
...
(function() {
  ...
  function updatePomodoroDisplay() {
    ...
    document.title = pomoInterval ? `(${timeStr}) Little Bloom Focus` : 'Little Bloom Pro 🌷';
  ...
  async function requestNotificationPermission() {
    ...
    if (perm === 'granted') {
      new Notification('Little Bloom Pro 🌷', {
  ...
  function exportDailyAgendaMarkdown() {
    ...
    md += `*Generated by Little Bloom Pro*\n\n`;
    md += `## ☀️ Active Tasks\n\n`;
  ...
  function triggerInstallPrompt() {
    ...
    deferredPrompt.userChoice.then((choiceResult) => {
      if (choiceResult.outcome === 'accepted') {
        showToast('Thank you for installing Little Bloom! 🌸', '🎉');
      }
      deferredPrompt = null;
    });
  }
    ...
    if ('serviceWorker' in navigator) {
      window.addEventListener('load', () => {
        navigator.serviceWorker.register('./sw.js')
          .then(reg => console.log('Little Bloom Service Worker registered:', reg.scope))
